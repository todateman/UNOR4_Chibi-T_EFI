...
- **噴射時間・点火進角・噴射終了角の 3 列とも**、グラフの点をドラッグして編集。  
  選択範囲に ±5% / ±1 段 / 線形補間 / 平滑化。グラフは横に並ぶので、調整しながら右の MAP テーブルが常に見える  
  （噴射終了角のグラフは既定で非表示。`噴射終了角のグラフ` にチェックで 3 枚目が出る）
- **噴射終了角は 720CA で周回する角度として扱う**。  
  線形補間・平滑化・差分表示・ライブ適用の変化量判定はすべて近い方の回りで測る  
  （680CA と 20CA はクランク角では 60CA しか離れていない。素の引き算だと既定 MAP の `20 → 680` の境界で補間が逆回りしてしまう）
...
- `calculatedINJ_END_CA` は **MAP 範囲外でもゼロクリアされない。**  
  ...
  噴射自体は `mapOutOfRange` で止まるので実害は無いが、値としては前回値が残るため GUI は `row=255` のとき表示しない。
...
- [x] `MAP SAVE`（停止時）中に `Ne_deg` のカウント抜けが無い（未検証、理由は検証結果ドキュメント参照）
...
- [x] 実走行（車速センサ `WH_IN`・実負荷・実燃料噴射量）は Ardu-Stim では代替不可のため未検証。実車で確認すること。
